...
class TransportService {
  ...
  discoverDevices() {
    ...
    const combined = Array.from(new Set([...activeDiscovered, ...baseUnits])).filter(id => id !== this.deviceId);
    return combined;
  }
    ...
    switch (data.type) {
      ...
      case 'CONNECT_REQUEST': {
        if (data.target_id === this.deviceId) {
          ...
          this._notify('status', { status: 'connected', device: data.sender_id });
      ...
      case 'DISCONNECT_NOTICE': {
        if (data.target_id === this.deviceId || this.connectedDevice === data.sender_id) {
          this.connectionStatus = 'disconnected';
          this.connectedDevice = null;
          this._notify('status', { status: 'disconnected', device: null });
        }
