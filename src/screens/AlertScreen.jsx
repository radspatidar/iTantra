  ...
  return (
    <div className="flex items-center justify-center gap-[3px] h-10">
      {bars.map((h, i) => (
        <div
          key={i}
          className={`w-1 rounded-full bg-red-400 transition-transform ${
            active ? "waveform-bar" : ""
          }`}
          style={{
            height: `${h * 32}px`,
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 overflow-y-auto min-h-0 px-4 py-5 flex flex-col gap-4 scroll-smooth">
        {/* Priority badge */}
        <div
          className={`rounded-2xl border-2 border-red-500 p-4 ${
            dark ? "bg-red-950/50" : "bg-red-50"
          }`}
        >
          <div className="flex items-center gap-2 mb-3">
            <div className="w-2.5 h-2.5 rounded-full bg-red-500 animate-emergency-pulse" />
            <span className="text-red-600 dark:text-red-400 text-xs font-bold tracking-widest uppercase">
          ...
          >
            ...
            >
              Received from{" "}
              <span className="font-bold text-red-500">{sender}</span>
            </p>
            <p
          ...
          <div className="flex items-center gap-3">
            <div
              className={`rounded-lg px-3 py-1.5 flex items-center gap-2 ${
                dark ? "bg-red-900/60" : "bg-red-100"
              }`}
            >
              <div
                className={`w-1.5 h-1.5 rounded-full ${
                  speaking
                    ? "bg-green-400 animate-emergency-pulse"
                    : "bg-slate-400"
                }`}
              />
              <span
              ...
              >
                {speaking
                  ? `Speaking Now · ${lang.label} voice`
                  : `Playback complete · ${lang.label}`}
              </span>
            </div>
          </div>
