  ...
  return (
    <div className="flex items-end gap-0.5">
      {[1, 2, 3].map(i => (
        <div
          key={i}
          className={`w-1 rounded-sm ${i <= active ? 'bg-teal-500' : 'bg-slate-600'}`}
...
export default function ConnectionScreen({
  transport, setTransport, connectedDevice, connectionStatus, onConnect, onDisconnect, dark, deviceId, setDeviceId
}) {
  const [selectedDevice, setSelectedDevice] = useState(null);
  const [nearbyDevices, setNearbyDevices] = useState(() => transportService.discoverDevices());
  ...
  return (
    <div className={`flex flex-col h-full ${dark ? 'bg-[#0F172A]' : 'bg-[#F0F4F8]'}`}>
      <div className={`flex-shrink-0 px-5 pt-5 pb-4 border-b ${divider} ${dark ? 'bg-slate-900' : 'bg-white'}`}>
        ...
        <p className={`text-xs mt-0.5 ${muted}`}>Offline peer-to-peer · No internet required</p>
      </div>
      ...
      <div className="flex-1 overflow-y-auto min-h-0 px-4 py-4 flex flex-col gap-3 scroll-smooth">
        ...
        <div className={`rounded-2xl border ${card} p-4`}>
          <p className={`text-[11px] font-bold tracking-widest uppercase mb-2 ${muted}`}>This Phone's Identity</p>
        ...
        {/* Transport selector */}
        <div className={`rounded-2xl border ${card} p-4`}>
          <p className={`text-[11px] font-bold tracking-widest uppercase mb-3 ${muted}`}>Transport Protocol</p>
          <div className="flex gap-2">
        ...
        {/* Current connection */}
        <div className={`rounded-2xl border ${card} p-4`}>
          <p className={`text-[11px] font-bold tracking-widest uppercase mb-3 ${muted}`}>Connected Device</p>
          {connectionStatus === 'connected' && connectedDevice ? (
            <div>
              <div className="flex items-center justify-between mb-3">
                <div>
                  <p className={`text-base font-bold ${text}`}>{connectedDevice}</p>
                  <p className={`text-xs ${muted}`}>{transport} · Strong Signal</p>
                </div>
              ...
              <div className="flex items-center gap-2 mb-3">
                ...
                <span className="text-green-500 text-sm font-semibold">Active Mesh Connection</span>
              </div>
              <button
            ...
            <div className="flex items-center gap-3 py-2">
              ...
              <span className={`text-sm ${muted}`}>Connecting to {selectedDevice || 'device'}...</span>
            </div>
          ) : (
        ...
        {/* Nearby devices */}
        <div className={`rounded-2xl border ${card} p-4`}>
          <p className={`text-[11px] font-bold tracking-widest uppercase mb-3 ${muted}`}>
            Nearby Devices · {transport}
          </p>
          <div className="flex flex-col gap-2">
            {nearbyDevices.map((device, i) => {
              const strengths = ['strong', 'medium', 'weak'];
              const isConnected = connectedDevice === device && connectionStatus === 'connected';
              return (
                <div
                  key={device}
                  className={`flex items-center justify-between p-3 rounded-xl border ${
                    isConnected
                      ...
                      : dark ? 'border-slate-700 bg-slate-700/50' : 'border-slate-200 bg-slate-50'
                  }`}
                >
                  <div className="flex items-center gap-2.5">
                    <div className={`w-2 h-2 rounded-full ${isConnected ? 'bg-green-500' : 'bg-slate-400'}`} />
                    <div>
                      <p className={`text-sm font-semibold ${text}`}>{device}</p>
                      <p className={`text-[11px] ${muted}`}>{transport} · Available</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
                    ...
                    {!isConnected && connectionStatus !== 'connecting' && (
                      <button
                        onClick={() => { setSelectedDevice(device); onConnect(device); }}
                    ...
                    {isConnected && (
                      <span className="text-xs font-semibold text-teal-600 ml-2">Active</span>
                    )}
                  </div>
                </div>
              );
        ...
        {/* Info note */}
        <div className={`rounded-xl p-3 ${dark ? 'bg-slate-800/50' : 'bg-blue-50'} border ${dark ? 'border-slate-700' : 'border-blue-200'}`}>
          <p className={`text-[11px] leading-relaxed ${dark ? 'text-slate-400' : 'text-blue-700'}`}>
            <span className="font-bold">Offline P2P Mesh:</span> Communication operates over local Bluetooth Low Energy / Wi-Fi Direct transport. No internet connection or cloud servers are required.
          </p>
        </div>
      </div>
    </div>
