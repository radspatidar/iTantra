...
const INITIAL_DARK = (() => {
  try {
    return localStorage.getItem("itantra-dark") === "true"
  } catch {
    return false
  }
})()
...
export default function App() {
  ...
  const setDark = (v) => {
    setDarkRaw(v)
    try {
      localStorage.setItem("itantra-dark", String(v))
    } catch {}
  }
  ...
  useEffect(() => {
      ...
      if (packet.priority === "emergency") {
        ...
        setTimeout(() => {
          setIncomingMessage((prev) =>
            prev ? { ...prev, isSpeaking: false } : null,
          )
        }, 3500)
      }
    }
    ...
    const handleIncomingAck = (ackPacket) => {
      if (
        ackPacket &&
        ackPacket.ref_packet_id === lastSentPacketIdRef.current
      ) {
  ...
  const handleEndTalk = useCallback(() => {
    ...
    after(800, () => {
      ...
      after(700, () => {
        ...
        after(4000, () => {
          setTalkState((prev) =>
            prev === "delivered" || prev === "acknowledged" ? "idle" : prev,
          )
        })
      })
    })
  ...
  return (
    <div
      className={`min-h-screen flex items-center justify-center sm:p-4 select-none ${
    ...
    >
      ...
      >
        ...
        {/* Bottom nav */}
        <div
          className={`flex-shrink-0 flex border-t z-20 ${
            ...
            },
            {
              ...
              icon: (
                <svg width="22" height="22" viewBox="0 0 24 24" fill="none">
                  ...
                  />
                  <path
                    d="M12 2v2m0 16v2M2 12h2m16 0h2m-3.5-7.5-1.5 1.5m-9 9-1.5 1.5m12 0-1.5-1.5m-9-9L4.5 4.5"
                    stroke="currentColor"
                    strokeWidth="2"
                    strokeLinecap="round"
                  />
